...
export class EventEditComponent implements OnInit {
  ...
  protected readonly form = this.fb.group({
    ...
    nature: ['CONQUETE' as NatureEvenement],
    type: ['ESSAI' as TypeEvenement],
  ...
  protected onNumeroJoueur1Change(numeroJoueur1: number | null): void {
    this.event.update(ev => ev ? { ...ev, numeroJoueur1: numeroJoueur1 ?? undefined } : undefined);
  }

  protected onNumeroJoueur2Change(numeroJoueur2: number | null): void {
    this.event.update(ev => ev ? { ...ev, numeroJoueur2: numeroJoueur2 ?? undefined } : undefined);
  }

  protected onDistanceJeuPiedChange(distanceJeuPied: number | null): void {
    this.event.update(ev => ev ? { ...ev, distanceJeuPied: distanceJeuPied ?? undefined } : undefined);
  }

  protected async save(): Promise<void> {
    await this.persistEvent(false);
  }

  protected async saveAndCreateAnother(): Promise<void> {
    await this.persistEvent(true);
  }

  private async persistEvent(createAnother: boolean): Promise<void> {
    ...
    this.error.set(null);
    try {
      const event = { ...currentEvent, ...this.form.getRawValue() } as Evenement;
      if (this.isEdit()) {
...
const ZONE_TERRAIN_OPTIONS: Option<ZoneTerrain>[] = [
  { value: 'NOS_22', label: 'Nos 22m' },
  { value: 'NOS_22_50', label: 'Nos 22m - 50m' },
  { value: 'LEUR_50_22', label: '50m - Leurs 22m' },
  { value: 'LEUR_22', label: 'Leurs 22m' },
] as Option<ZoneTerrain>[];

const POSITION_LARGEUR_OPTIONS: Option<PositionLargeur>[] = [
...
const COMPLEMENT_DISCIPLINE_OPTIONS: Option<ComplementDiscipline>[] = [
...
const FAUTES_PENALITE_OPTIONS: Option<FautesPenalite>[] = [
  ['HORS_JEU', 'Hors-jeu'],
...
const FAUTES_BRAS_CASSE_OPTIONS: Option<FautesBrasCasse>[] = [
...
const CHOIX_DE_JEU_PENALITE_OPTIONS: Option<ChoixDeJeuPenalite>[] = [
...
const CHOIX_DE_JEU_BRAS_CASSE_OPTIONS: Option<ChoixDeJeuBrasCasse>[] = [
  { value: 'MAIN', label: 'Main', icon: '/icons/event/MAIN.png' },
  { value: 'MELEE', label: 'Mêlée', icon: '/icons/event/MELEE.png' },
  { value: 'TOUCHE', label: 'Touche', icon: '/icons/event/TOUCHE.png' },
];

const RESULTAT_MELEE_OPTIONS: Option<ResultatMelee>[] = [
...
const RESULTAT_MAUL_OPTIONS: Option<ResultatMaul>[] = [
...
const RESULTAT_RUCK_OPTIONS: Option<ResultatRuck>[] = [
...
const RESULTAT_TOUCHE_OPTIONS: Option<ResultatTouche>[] = [
  ['GAGNEE', 'Gagnée', '/icons/event/REUSSITE.png'], 
  ['PERDUE', 'Perdue', '/icons/event/ECHEC.png'], 
  ['CONTRE', 'Contre', '/icons/event/ECHEC.png'], 
  ['PENALISEE', 'Pénalisée', '/icons/event/ECHEC.png'], 
  ['BRAS_CASSE', 'Bras cassé', '/icons/event/ECHEC.png']
  ].map(([value, label, icon]) => ({ value, label, icon })) as Option<ResultatTouche>[];

const RECUPERATION_OPTIONS: Option<Recuperation>[] = [
  ['GAGNE', 'Gagné', '/icons/event/REUSSITE.png'],
  ['PERDU', 'Perdu', '/icons/event/ECHEC.png']
  ].map(([value, label, icon]) => ({ value, label, icon })) as Option<Recuperation>[];

const RESULTAT_OPTIONS: Option<Resultat>[] = [
...
const EVENT_NATURE_OPTIONS : Option<NatureEvenement>[] = [
